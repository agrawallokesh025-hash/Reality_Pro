...
export function WhatsAppButton() {
  ...
  React.useEffect(() => {
    ...
    async function loadSettings() {
      try {
        ...
        console.error("Failed to load settings for WhatsApp button", err)
      }
    }
    loadSettings()
  }, [pathname])

  if (!settings || !settings.whatsapp_number || pathname?.startsWith("/dashboard")) {
    return null
  }

  const number = settings.whatsapp_number.replace(/[^0-9]/g, "")
  const defaultText = settings.whatsapp_template || "Hello! I would like to inquire about properties."
  ...
  return (
    <a
      ...
      className="fixed bottom-6 right-6 z-50 flex h-14 w-14 items-center justify-center rounded-full bg-emerald-500 text-white shadow-lg shadow-emerald-500/20 hover:bg-emerald-400 hover:scale-105 transition-all duration-300 group cursor-pointer"
