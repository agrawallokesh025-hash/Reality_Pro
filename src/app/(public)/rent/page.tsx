      ...
      <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
        {[1, 2, 3, 4, 5, 6].map((i) => (
          <div key={i} className="rounded-lg border bg-card shadow-sm">
            ...
            <div className="p-6">
              <h3 className="font-semibold text-xl">Luxury Apartment {i}</h3>
              <p className="text-sm text-muted-foreground mt-2">123 Example Street, NY</p>
              <div className="mt-4 flex items-center justify-between">
                <span className="font-bold text-lg">${3000 + i * 500}/mo</span>
                <Button variant="outline" size="sm" asChild>
                  <Link href={`/properties/apartment-${i}`}>Details</Link>
                </Button>
              </div>
            </div>
          </div>
